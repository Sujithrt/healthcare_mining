...
if 'mistral_chat_history' not in st.session_state:
    st.session_state['mistral_chat_history'] = []
...
def main():
    ...
    st.header('Mistral Healthcare Chatbot')
    with st.sidebar:
    ...
    conversational_retrieval_chain_with_mistral = create_conversational_retrieval_chain(astra_vector_store,
    ...
    context = App.select_context(conversational_retrieval_chain_with_mistral)
    ...
    mistral_conversational_retrieval_chain_recorder = TruChain(
        conversational_retrieval_chain_with_mistral,
        app_id="Conversation-Retrieval-Chain-feedback-Mistral",
        feedbacks=[f_qa_relevance, f_context_relevance, f_no_answer, f_answer]
    )

    if user_input := st.chat_input("Ask me anything"):
        st.session_state.mistral_messages.append(HumanMessage(content=user_input))
        st.chat_message("user").markdown(user_input)
        with mistral_conversational_retrieval_chain_recorder as recording:
            response_data = conversational_retrieval_chain_with_mistral({
                "question": user_input,
                "chat_history": st.session_state["mistral_chat_history"]
            })
            print(response_data)
            st.session_state["mistral_chat_history"].append((user_input, response_data["answer"]))
            st.chat_message("assistant").markdown(response_data["answer"])
            st.session_state.mistral_messages.append(AIMessage(content=response_data["answer"]))
            if len(st.session_state.mistral_messages) > 25:
                st.session_state.mistral_messages = st.session_state.mistral_messages[-25:]
