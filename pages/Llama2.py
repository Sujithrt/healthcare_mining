...
if "llama2_messages" not in st.session_state:
    st.session_state.llama2_messages = []
...
def main():
    ...
    st.header('Llama2 Healthcare Chatbot')
    with st.sidebar:
    ...
    for message in st.session_state.llama2_messages:
    ...
    conversational_retrieval_chain_with_llama = create_conversational_retrieval_chain(astra_vector_store,
    ...
    context = App.select_context(conversational_retrieval_chain_with_llama)
    ...
    llama2_conversational_retrieval_chain_recorder = TruChain(
        conversational_retrieval_chain_with_llama,
        app_id="Conversation-Retrieval-Chain-feedback-Llama2",
        feedbacks=[f_qa_relevance, f_context_relevance, f_no_answer, f_answer]
    )

    if user_input := st.chat_input("Ask me anything"):
        st.session_state.llama2_messages.append(HumanMessage(content=user_input))
        st.chat_message("user").markdown(user_input)
        with llama2_conversational_retrieval_chain_recorder as recording:
            response_data = conversational_retrieval_chain_with_llama({
                "question": user_input,
                "chat_history": st.session_state["llama2_chat_history"]
            })
            print(response_data)
            st.session_state["llama2_chat_history"].append((user_input, response_data["answer"]))
            st.chat_message("assistant").markdown(response_data["answer"])
            st.session_state.llama2_messages.append(AIMessage(content=response_data["answer"]))
            if len(st.session_state.llama2_messages) > 25:
                st.session_state.llama2_messages = st.session_state.llama2_messages[-25:]
